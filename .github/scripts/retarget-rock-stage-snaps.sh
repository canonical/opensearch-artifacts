...
if ! yq \
    '.parts[] | select(has("stage-snaps")) | .["stage-snaps"][] | select(test("^" + env.ROCK_NAME + "/3/edge$"))' \
    "${ROCKCRAFT_FILE}" | grep -q .; then
  echo "No ${ROCK_NAME}/3/edge stage snap channel found in ${ROCKCRAFT_FILE}"
  exit 1
fi
...
yq -Yi \
  '(.parts[] | select(has("stage-snaps")) | .["stage-snaps"][]) |= sub("/3/edge$"; "/" + env.SNAP_CHANNEL)' \
  "${ROCKCRAFT_FILE}"
