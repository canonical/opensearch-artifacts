...
class ConfigSetter:
    ...
    def put(
        ...
        self.__dump(
            data, output_type, config_file if output_file is None else output_file
        )
...
def parse_args():
    ...
    def __parse():
        parser = ArgumentParser(
            description="Utility script for editing YAML conf files."
        )
        ...
        parser.add_argument(
            "-k", "--key", type=str, required=True, help="Key of the element to target."
        )
...
if __name__ == "__main__":
    ...
    if args.delete:
        conf_setter.delete(args.file, args.key, sep=args.sep, output_type=args.out)
    else:
        conf_setter.put(
            args.file,
            args.key,
