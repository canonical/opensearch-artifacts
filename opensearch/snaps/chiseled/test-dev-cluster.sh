...
function run_tests () {
    ...
    sudo snap run opensearch-chiseled.test-cluster-health-green --admin-auth-password "${admin_auth_password}"

    echo -e "\n\n---------------\n\n"

    # Check if node is up:
    echo "Running: test-node-up..."
    sudo snap run opensearch-chiseled.test-node-up --admin-auth-password "${admin_auth_password}"

    echo -e "\n\n---------------\n\n"

    # Check if the security index is well initialised:
    echo "Running: test-security-index-created..."
    sudo snap run opensearch-chiseled.test-security-index-created --admin-auth-password "${admin_auth_password}"
}
