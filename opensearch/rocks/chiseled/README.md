...
This repository contains the packaging metadata for creating the Chiseled OpenSearch rock. This rock image is based on the [opensearch-chiseled Snap](https://github.com/canonical/opensearch-artifacts/tree/add-chiseled-3-edge-rock/opensearch/snaps/chiseled).

For more information on rocks, visit the [rockcraft Github](https://github.com/canonical/rockcraft).

## Bundled plugins

Unlike the standard and charmed rocks, this image ships only the `opensearch-security` plugin. Every other bundled plugin is removed while building the [opensearch-chiseled Snap](https://github.com/canonical/opensearch-artifacts/tree/add-chiseled-3-edge-rock/opensearch/snaps/chiseled), including `opensearch-knn` and `opensearch-performance-analyzer`. As a consequence this rock does not relocate the k-NN native libraries and does not add them to `java.library.path`.
